...
def executar_analise_llm(
    ...
    """
    Executa uma análise utilizando um modelo LLM da OpenAI.

    Args:
        tipo_analise (str): Tipo de análise a ser realizada (ex: 'design', 'pentest').
        codigo (str): Código-fonte a ser analisado.
        analise_extra (str): Instruções extras fornecidas pelo usuário.
        model_name (str): Nome do modelo da OpenAI a ser utilizado.
        max_token_out (int): Número máximo de tokens para a resposta.

    Returns:
        str: Resultado da análise retornado pelo modelo LLM.

    Raises:
        RuntimeError: Se ocorrer uma falha na comunicação com a OpenAI.
    """
