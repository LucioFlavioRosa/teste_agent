...
def connect_to_repo(repositorio: str, token: str):
    """Realiza conexão com o repositório GitHub usando token."""
    auth = Token(token)
    g = Github(auth=auth)
...
def main(repo, tipo_de_analise: str):
    ...
    repositorio_final = connect_to_repo(repositorio=repo, token=token)
