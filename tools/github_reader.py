import re
from github import Github
from github.Auth import Token
from google.colab import userdata

def obter_token_github():
    """
    Obtém o token do GitHub de forma desacoplada para facilitar testes.
    """
    return userdata.get('github_token')

def conection(repositorio: str, token: str = None):
    """
    Conecta ao repositório do GitHub usando um token fornecido ou padrão.
    """
    if token is None:
        token = obter_token_github()
    auth = Token(token)
    g = Github(auth=auth)
    return g.get_repo(repositorio)

MAPEAMENTO_TIPO_EXTENSOES = {
    "terraform": [".tf", ".tfvars"],
    "python": [".py"],
    "cloudformation": [".json", ".yaml", ".yml"],
    "ansible": [".yml", ".yaml"],
    "docker": ["Dockerfile"],
}

def deve_ler_arquivo(conteudo, extensoes):
    """
    Decide se o arquivo deve ser lido de acordo com as extensões alvo.
    """
    if extensoes is None:
        return True
    return (
        any(conteudo.path.endswith(ext) for ext in extensoes)
        or conteudo.name in extensoes
    )

def decodificar_conteudo(conteudo):
    """
    Decodifica o conteúdo do arquivo, tratando exceções localmente.
    """
    try:
        return conteudo.decoded_content.decode('utf-8')
    except Exception as e:
        print(f"DEBUG: ERRO na decodificação de '{conteudo.path}': {e}")
        return None

def leitura_recursiva(repo, extensoes, path="", arquivos_do_repo=None):
    """
    Lê arquivos de um repositório de forma recursiva, filtrando por extensão.
    """
    if arquivos_do_repo is None:
        arquivos_do_repo = {}
    try:
        conteudos = repo.get_contents(path)
        for conteudo in conteudos:
            if conteudo.type == "dir":
                leitura_recursiva(repo, extensoes, conteudo.path, arquivos_do_repo)
            else:
                if deve_ler_arquivo(conteudo, extensoes):
                    codigo = decodificar_conteudo(conteudo)
                    if codigo is not None:
                        arquivos_do_repo[conteudo.path] = codigo
    except Exception as e:
        print(e)
    return arquivos_do_repo

def main(repo, tipo_de_analise: str):
    """
    Função principal para leitura de arquivos do repositório conforme tipo de análise.
    """
    repositorio_final = conection(repositorio=repo)
    extensoes_alvo = MAPEAMENTO_TIPO_EXTENSOES.get(tipo_de_analise.lower())
    arquivos_encontrados = leitura_recursiva(
        repositorio_final,
        extensoes=extensoes_alvo
    )
    return arquivos_encontrados
