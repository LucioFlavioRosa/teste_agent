...
def conectar_ao_github(repositorio_nome: str, config: Optional[Config] = None):
    try:
        github_token = None
        ...
        if not github_token:
            try:
                ...
                pass
        if not github_token:
            logging.error("Token do GitHub não encontrado em config, variáveis de ambiente ou userdata.")
            raise ValueError("Token do GitHub não encontrado.")
        auth = Token(github_token)
        github_client = Github(auth=auth)
        repositorio = github_client.get_repo(repositorio_nome)
...
def ler_arquivos_repositorio_github(repositorio_nome: str, tipo_analise: str, max_workers: int = MAX_PARALLELISM, max_depth: Optional[int] = None, config: Optional[Config] = None):
    try:
        repositorio = conectar_ao_github(repositorio_nome=repositorio_nome, config=config)
        extensoes_alvo = TIPO_EXTENSOES_MAPEAMENTO.get(tipo_analise.lower())
        arquivos_encontrados = leitura_iterativa_com_paralelismo_e_retry(repositorio, extensoes_alvo, max_workers=max_workers, max_depth=max_depth)
        logging.info(f"Arquivos encontrados para análise '{tipo_analise}': {list(arquivos_encontrados.keys())}")
        return arquivos_encontrados
...
def obter_arquivos_para_analise(repo_nome: str, tipo_analise: str, max_workers: int = MAX_PARALLELISM, max_depth: Optional[int] = None, config: Optional[Config] = None):
    return ler_arquivos_repositorio_github(repo_nome, tipo_analise, max_workers=max_workers, max_depth=max_depth, config=config)
