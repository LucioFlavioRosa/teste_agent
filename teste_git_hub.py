...
Script de demonstração para execução de análises via agente LLM e servidor Flask.
Inclui exemplo de uso interativo e endpoints REST para integração com agentes de revisão.

Observação: Recomenda-se separar a lógica de teste interativo e aplicação Flask em arquivos distintos para melhor manutenção.
"""
...
def rodar_analise() -> tuple:
    """
    Endpoint para executar análise de código via agente LLM.

    Returns:
        tuple: (json, status_code) com resultado da análise ou mensagem de erro.
    """
