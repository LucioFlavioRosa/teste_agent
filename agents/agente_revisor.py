...
class Config:
    def __init__(self, github_token: Optional[str] = None, openai_api_key: Optional[str] = None):
        # Prioriza variáveis de ambiente para facilitar testes e CI/CD
...
def obter_codigo_repositorio(repositorio_nome: str, tipo_analise: str, config: Optional[Config] = None) -> Dict[str, str]:
    try:
        logging.info(f'Iniciando a leitura do repositório: {repositorio_nome}')
        arquivos_codigo = github_reader.obter_arquivos_para_analise(repo_nome=repositorio_nome, tipo_analise=tipo_analise, config=config)
        return arquivos_codigo
    except (ValueError, RuntimeError) as e:
        logging.error(f"Falha ao executar a análise de '{tipo_analise}': {e}")
        raise
...
def preparar_codigo_para_analise(tipo_analise: str, repositorio_nome: Optional[str], codigo_entrada: Optional[Union[str, Dict[str, str]]], config: Optional[Config] = None):
    if codigo_entrada is not None:
        return codigo_entrada
    return obter_codigo_repositorio(repositorio_nome=repositorio_nome, tipo_analise=tipo_analise, config=config)
...
def executar_analise(tipo_analise: str,
    ...
    try:
        validar_parametros_entrada(tipo_analise=tipo_analise, repositorio_nome=repositorio, codigo_entrada=codigo_entrada)
        codigo_para_analise = preparar_codigo_para_analise(tipo_analise=tipo_analise, repositorio_nome=repositorio, codigo_entrada=codigo_entrada, config=config)
        if not codigo_para_analise:
            logging.warning('Não foi fornecido nenhum código para análise.')
            return {"tipo_analise": tipo_analise, "resultado": 'Não foi fornecido nenhum código para análise'}
        codigo_final = montar_codigo_para_llm(codigo_para_analise)
